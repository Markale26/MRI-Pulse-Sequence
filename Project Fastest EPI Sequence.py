...
# chk with max_grad and max_slew. research google and tell practical and actual discrepencies. fill the gaps as much as possible
...
rf1, _, _ = pp.make_sinc_pulse(
    flip_angle=90 * np.pi / 180, duration=0.5e-3,
    slice_thickness=slice_thickness, apodization=0.3, time_bw_product=2.5,
    system=system, return_gz=True
)


# Define other gradients and ADC events
gx = pp.make_trapezoid(channel='x',rise_time=0.00001,area=Nread, duration=0.1e-3, system=system)
gx_ = pp.make_trapezoid(channel='x', area=-Nread,rise_time=0.00001, duration=0.1e-3, system=system)
adc = pp.make_adc(num_samples=Nread, duration=0.1e-3, phase_offset=0 * np.pi / 180, system=system)

gx_pre = pp.make_trapezoid(channel='x', area=-gx.area/2, duration=1e-3, system=system)
gy_pre = pp.make_trapezoid(channel='y', area=-gx.area/2, duration=1e-3, system=system)

gp = pp.make_trapezoid(channel='y', area=1, rise_time=0.000001,duration=0.1e-3, system=system)
...
for ii in range(0, Nphase // 2):  # e.g. 0,16
...
graph = mr0.compute_graph(seq0, obj_p, 100, 1e-3)
